...
import { generateCustomTest } from '@/ai/flows/custom-test-generator';
import { CustomTestInputSchema } from '@/ai/schemas/custom-test-schemas';
...
export async function POST(request: Request) {
  try {
    ...
    const validation = CustomTestInputSchema.safeParse(body);

    if (!validation.success) {
        return NextResponse.json({ error: 'Invalid input for custom test generation.', details: validation.error.flatten() }, { status: 400 });
    }
    
    const test = await generateCustomTest(validation.data);
    ...
    const errorMessage = error.message && error.message.includes('GEMINI_API_KEY')
      ...
      : 'An unexpected server error occurred while generating the custom test.';
